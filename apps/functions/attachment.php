 ...
 * @return string
 */
function get_image($id = 0 , $type='',$txtype='') {
    ...
    if (!$url){
        if($txtype=='headimg'){
            $url = config('index_url').'/static/assets/img/avatar-default.png';
        }else{
            $url = config('index_url').'/static/assets/img/noimage.gif';
        }
    }
 ...
 * @return string
 */
function get_attachment_info($id,$field='') {
    if ((int)$id) {
        $info = AttachmentLogic::info($id);
        return !empty($field) ? $info[$field] : $info;
    }
    return false;
}
...
/**
*导出excel
 * $title:表格名称，$thead：表格头部标题，$list:导出的数据
 */
function downExcelExport($thead=array(),$list=array(),$title=''){
    /** Error reporting */
    ...
    //循环表格头部标题
    foreach ($thead as $ks=>$vs){
        $objPHPExcel->getActiveSheet()->SetCellValue(bigABC($ks).'1', $vs['title']);
    }

    $ii=0;
   foreach($list as $k=>$v){
        foreach ($thead as $kt=>$vt) {
            $objPHPExcel->getActiveSheet()->SetCellValue(bigABC($kt) . ($k+2),' '.$v[$vt['ziduan']]);
        }
    }
    $objPHPExcel->getActiveSheet()->setTitle($title.date('Y-m-d'));
