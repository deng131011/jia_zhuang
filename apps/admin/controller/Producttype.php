...
class Producttype extends Admin {
    ...
    public function index(){
        ...
        $extra_html = logic('Base')->moveMenuHtmlCom('Producttype');//添加移动按钮html
        $content = builder('List')
           ...
           // ->addTopButton('self', $move_position_attr) //移动菜单位置
            ->addTopBtn('sort',array('model'=>'Producttype','href'=>url('Sort')))  // 添加排序按钮
            ->setSearch('请输入关键字')
            ->keyListItem('id','ID编号')
            ->keyListItem('title_show','名称')
            ->keyListItem('flag','推荐','array',[1=>'推荐',2=>''])
    ...
    public function edit($id=0){
        ...
        if(IS_POST){
            ...
            $data['level'] = $this->Producttype->level($data['pid']);
            ...
            $content = builder('form')
                    ...
                    ->addFormItem('flag', 'radio', '推荐分类', '二级分类首页处推荐',[1=>'推荐',2=>'不推荐'])
    ...
    public function Sort($ids = null)
    {
        ...
        if (IS_POST) {
            ...
            $builder->doSort('producttype', $ids);
        } else {
            $map['status'] = 1;
            $list = $this->Producttype->getList($map,'id,title,sort,pid','sort asc,id asc');
    ...
    public function moveMenusPosition() {
        if (IS_POST) {
            ...
            if ($to_pid || $to_pid==0) {
                $result = logic('Base')->moveMenusPositionCom($ids,$to_pid,'Producttype');
                if ($result) {
