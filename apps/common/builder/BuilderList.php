    ...
    public function addBtn($position='top',$type, $attribute = null){
        ...
        switch ($type) {
            ...
            case 'self': //添加自定义按钮(第一原则使用上面预设的按钮，如果有特殊需求不能满足则使用此自定义按钮方法)
                ...
                $my_attribute['target-form'] = 'ids';
                $my_attribute['class'] = 'btn btn-success btn-sm';
                break;
        }
        /**
        * 如果定义了属性数组则与默认的进行合并
        ...
        foreach ($this->tableColumns as &$column) {
            ...
            switch ($column_type) {
                ...
                case 'picture':
                    ...
                    if (!empty($column['param']['style'])) {
                        $style = $column['param']['style'];
                    }
                    $data[$column['name']] = '<img class="picture" width="'.$pic_w.'" src="'.get_image($data[$column['name']]).'" style="'.$style.'" >';
                    break;
                case 'pictures':
                    $pic_w = '120';
                    if (!empty($column['param']['width'])) {
