...
class Upload {
	...
	public function upload($param = []) {
		...
		try {
		    if(!empty($param['type']) && $param['type']=='file'){
                $upload_type = $this->request->param('type', 'file', 'trim');//上传类型包括picture,file,avatar

            }else{
                $upload_type = $this->request->param('type', 'picture', 'trim');//上传类型包括picture,file,avatar

            }
            $this->path_type = $this->request->param('path_type', 'file', 'trim');//路径类型
            ...
            $fileval = !empty($param['type']) ? $param['type'] : 'file';
           // return $fileval.'-'.$upload_type;
