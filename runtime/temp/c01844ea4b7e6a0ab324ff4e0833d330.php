<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:77:"G:\phpstudy\www\jia_zhuang\public_html/../apps/admin\view\product\gui_ge.html";i:1613802486;s:74:"G:\phpstudy\www\jia_zhuang\public_html/../apps/admin/view/public/base.html";i:1563871977;s:46:"../apps/admin/view/public/document_header.html";i:1564739010;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
  <head>
    <meta charset="utf-8">
        ...
        <form id="search_form" action="<?php echo url(''); ?>" method="get">
            <div class="box-body" id="eacoo-toolbar">
                <div class="form-inline" role="form">
                    ...
                    <div class="form-group mt-10">
                        <input type="text"  class="form-control" name="keywords" value="<?php echo (isset($get['keywords']) && ($get['keywords'] !== '')?$get['keywords']:''); ?>" placeholder="关键词">
                    </div>
                    <input type="hidden" name="product_id" value="<?php echo $get['product_id']; ?>" />
                    <div class="form-group mt-10">
                        ...
                        <div class="bootstrap-table">
                            <div class="fixed-table-toolbar"><div class="bs-bars pull-left">
                                <div id="builder-toolbar" class="toolbar">
                                   ...
                                   <a title="添加" icon="fa fa-plus" class="btn btn-primary btn-sm" href="<?php echo url('gg_edit',array('product_id'=>$get['product_id'])); ?>" primary-key="id"><i class="fa fa-plus"></i> 添加</a>


                                    <a title="启用" target-form="ids" icon="fa fa-play" class="btn btn-success ajax-table-btn confirm btn-sm" href="<?php echo url('setstatus',array('status'=>'show','model'=>'ProductGuige')); ?>" data-my="1" primary-key="id"><i class="fa fa-play"></i> 启用</a>&nbsp;

                                    <a title="禁用" target-form="ids" icon="fa fa-pause" class="btn btn-warning ajax-table-btn confirm btn-sm" confirm-info="您确定要执行禁用操作吗？" href="<?php echo url('setstatus',array('status'=>'hide','model'=>'ProductGuige')); ?>" data-my="1" primary-key="id"><i class="fa fa-pause"></i> 禁用</a>&nbsp;
                                    <a title="删除" target-form="ids" icon="fa fa-remove" class="btn btn-danger ajax-table-btn confirm btn-sm" confirm-info="您确定要执行删除操作吗？" href="<?php echo url('setstatus',array('status'=>'delete','model'=>'ProductGuige')); ?>" data-my="1" primary-key="id"><i class="fa fa-remove"></i> 删除</a>
                                ...
                                <div class="fixed-table-container fixed_table_list" style="padding-bottom: 0px;">
                                    ...
                                    <table id="builder-table" class="table table-responsive table-bordered table-hover dataTable" data-pagination="true" data-toolbar="#builder-toolbar" data-query-params="queryParams" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" width="100%">
                                        ...
                                        <tr>
                                            ...
                                            <th style="width:20%;">
                                                <div class="th-inner sortable both">规格名称</div>
                                            </th>
                                            <th style="width:10%;">
                                                <div class="th-inner sortable both">规格类别</div>
                                            </th>
                                          <!--  <th style="width:10%;">
                                                <div class="th-inner sortable both">库存</div>
                                            </th>-->
                                            <th style="width:10%;">
                                                <div class="th-inner sortable both">成本价格</div>
                                            </th>
                                            <th style="width:10%;">
                                                <div class="th-inner sortable both">原价</div>
                                            </th>
                                            <th style="width:10%;">
                                                <div class="th-inner sortable both">普通价格</div>
                                            </th>
                                            <th style="" >
                                        ...
                                        <tr data-index="<?php echo $key; ?>" data-uniqueid="<?php echo $vo['id']; ?>">
                                            ...
                                            <td style=""><?php echo $vo['gg_title']; ?></td>
                                            <td style=""><?php echo modelField($vo['typeid'],"spec_type","title"); ?></td>

                                            <td style=""><?php if($vo['is_price'] == 1): ?><?php echo $vo['cben_price']; else: ?>--<?php endif; ?></td>
                                            <td style=""><?php if($vo['is_price'] == 1): ?><?php echo $vo['old_price']; else: ?>--<?php endif; ?></td>
                                            <td style=""><?php if($vo['is_price'] == 1): ?><?php echo $vo['new_price']; else: ?>--<?php endif; ?></td>
                                            <td style=""><?php echo forbidenStatus($vo['status']); ?></td>
                                            <td style="">
                                                <?php if($vo['status'] == 1): ?>
                                                <a class="btn btn-warning btn-xs ajax-get confirm" href="<?php echo url('setstatus',array('status'=>'hide','ids'=>$vo['id'],'model'=>'ProductGuige')); ?>">下架</a>
                                                <?php else: ?>
                                                <a class="btn btn-success btn-xs ajax-get confirm" href="<?php echo url('setstatus',array('status'=>'show','ids'=>$vo['id'],'model'=>'ProductGuige')); ?>">上架</a>
                                                <?php endif; ?>
                                                <a class="btn btn-success btn-xs" href="<?php echo url('gg_edit',array('id'=>$vo['id'])); ?>">编辑</a>
                                                <a title="删除" icon="fa fa-remove" class="btn btn-danger btn-xs ajax-get confirm" confirm-info="您确定要执行删除操作吗？" href="<?php echo url('setstatus',array('status'=>'delete','ids'=>$vo['id'],'model'=>'ProductGuige')); ?>" style="margin-right:6px;"><i class="fa fa-remove"></i> </a>
